= Project PLATEAU CityGML 3 Prototype sample documents
...
`sources/001-cg3pre/`::
PLATEAU Handbook #01 source, v6_draft.

`sources/iur/`::
i-UR 4.0 code list data. Downloaded from here:
...
`sources/002-cg3pre/`::
PLATEAU Handbook #02 source, v6_draft.
...
bundle
bundle exec metanorma sources/001-cg3pre/document.adoc
# => sources/001-cg3pre/document.{html,xml,pdf}
----
...
[source,sh]
----
bundle exec metanorma sources/001-cg3pre/document.adoc # or whichever document
----

This command outputs the built document at:

* `sources/001-cg3pre/document.{html,pdf,xml}`
...
[source,sh]
----
$ bundle exec reverse_adoc -rcoradoc/reverse_adoc/plugins/plateau --split-sections 2 --external-images -o sources/001-cg3pre/document.adoc reference-docs/plateau-document-01-cg3pre.html
$ bundle exec reverse_adoc -rcoradoc/reverse_adoc/plugins/plateau --split-sections 2 --external-images -o sources/002-cg3pre/document.adoc reference-docs/plateau-document-02-cg3pre.html
----
...
[source,sh]
----
sed -i '' 's/\.webp/.webp.png/g' sources/001-cg3pre/sections/*.adoc
----
...
- You executed the command `metanorma sources/001-cg3pre/document.adoc` beforehand.
- You want Firelight output to be in an existing subdirectory `firelight-build-001`.

First, create a config file with this command.
(Do not add this file in Git, it will only be used for local build
and can be deleted any time later.)
...
{
  "version": "0.1",
  "entryPoint": "file:documents/001-cg3pre/document.presentation.xml",
  "storeAdapters": [
...
Then, instruct Docker to run a Firelight build NPX command in a Node container
as follows.
...
Now you should be able to open `localhost:8888` in your browser
