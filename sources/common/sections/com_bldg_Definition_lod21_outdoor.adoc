...
|屋根の上方からの正射影の外周と、地表面と外壁面の交線の正射影との距離が1m未満の場合は、軒裏を表現しない。
...
|lod2MultiSurface
|GM_MultiSurface
|建築物外部に存在する付属物のうち、短辺が実長3m以上又は短辺実長1m以上かつ上方又は側方からの正射影の面積3m2以上
|
...
LOD2.1では1m未満の軒裏を表現せず、外壁面を屋根面の外周から垂直に下ろした面として表現する。開口部はこの外壁面上に作成されることから、外壁面等に対する正射影を取得する。
