...
[[fig-C-20]]
.BuildingPartの使用例
image::../../sources/images/EA_bldg_BuildingPart.png[width="500"]
...
例えば、<<fig-C-21>>のように、一棟の建築物が商業施設、業務施設、共同住宅からなる複合施設であり、それぞれの用途に対して都市計画基礎調査が実施されていたとする。この場合、商業施設、業務施設、共同住宅をそれぞれbldg:BuildingPartとして記述し、bldg:BuildingPartごとに都市計画基礎調査で得られた情報を属性として付与できる。
...
同様にして、bldg:BuildingPartはデータ品質属性（urc:DataQualityAttribute）をもつことができる。この場合、これを部品とするbldg:Buildingはデータ品質属性をもたない。また、データ品質属性の各属性には、bldg:BuildingPartのデータ作成に使用した原典資料の地図情報レベル、その他原典資料の諸元を記述することが基本となる。ただし、urc:geometrySrcDescLod0、urc:lod1HeightType等のLOD0及びLOD1の幾何オブジェクトについての属性は、このbldg:BuildingPartを部品として持つbldg:BuildingのLOD0及びLOD1の幾何オブジェクトに対応する値を記述する。

なお、bldg:Buildingの部品となる全てのbldg:BuildingPartについて、データ品質属性の値が全て等しくなる場合は、bldg:Buildingにデータ品質属性を記述することができる。このとき、部品となる各bldg:BuildingPartはデータ品質属性をもたない。

[[fig-C-21]]
.建築物部分（bldg:BuildingPart）を使用する例
image::../../sources/images/anx_C_021.png[]

ただし、ユースケースで必要ない場合には、bldg:BuildingPartを使用しなくてもよい。 +
例えば、<<fig-C-22>>のように、複合施設であっても主たる用途についてのみ都市計画基礎調査が実施されている場合で、用途ごとに棟を分けて取得する必要が無い場合には、bldg:BuildingPartを使用せず、bldg:Buildingのみを使用して記述する。

[[fig-C-22]]
.建築物部分（bldg:BuildingPart）を使用しない例
image::../../sources/images/anx_C_022.png[]
